...
const endMarkerMatch = file.match(/<\/div>\s*<\/div>\s*\)\s*:\s*\(/);

if (!endMarkerMatch || file.indexOf(startMarker) === -1) {
    console.error("Markers not found");
    process.exit(1);
}

const startIdx = file.indexOf(startMarker) + startMarker.length;
const endIdx = endMarkerMatch.index;
...
fs.writeFileSync('src/pages/ConfiguracoesPage.tsx', prefix + payload + "\n" + suffix);
