...
const target = `<Lock size={20} className="text-blue-600" /> Parâmetros do Sistema`;
...
if (insertIndex !== -1) {
    if (lines[insertIndex - 1] && lines[insertIndex - 1].includes('<div className="p-6')) {
         lines.splice(insertIndex - 1, 0, newCard);
    } else {
         lines.splice(insertIndex, 0, newCard);
    }
    fs.writeFileSync(filePath, lines.join('\n'), 'utf-8');
    console.log('SUCCESS AT INDEX ' + insertIndex);
} else {
    console.log('TARGET NOT FOUND IN INDEX LOOP');
}
