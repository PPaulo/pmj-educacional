...
if (content.includes(targetStr)) {
    ...
    // try agnostic approach
    const lines = content.split('\\n');
    let foundIndex = -1;
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('LOGRADOURO (Rua)') && lines[i+21] && lines[i+21].includes('p-6 bg-white')) {
             foundIndex = i + 21;
             break;
        }
    }
    if (foundIndex !== -1) {
         lines.splice(foundIndex, 0, `                             <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm space-y-4">
                                  <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><School size={20} className="text-blue-600" /> Infraestrutura e Atendimento</h3>
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                       <div className="space-y-2">
                                            <h4 className="text-xs font-bold text-slate-500 uppercase">Infraestrutura</h4>
         ...
         console.log('PATCH BY INDEX SUCCESSFUL!');
    } else {
         console.log('TARGET STRINGS NOT FOUND IN AGNOSTIC SEARCH EITHER!');
    }
}
